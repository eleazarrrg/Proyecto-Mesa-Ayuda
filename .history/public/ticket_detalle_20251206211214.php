...
// Solo el agente (rol 2) puede gestionar el ticket
$esAgente = ($roleId == 2);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$esAgente) {
        $errores[] = "No tienes permisos para gestionar este ticket.";
    } else {
        ...
        if (!in_array($estado, ['EN_ESPERA','EN_PROCESO','CULMINADA'])) {
        ...
        if ($estado === 'CULMINADA') {
            if (!$agenteId) {
                $errores[] = "Debe seleccionar un agente antes de marcar el ticket como culminado.";
            }
            if (!Validator::required($solucion)) {
                $errores[] = "Debe registrar la solución antes de culminar el ticket.";
            }
        }
...
<body>
    ...
    <?php else: ?>
        <p>Solo el agente de TI puede gestionar y cerrar el ticket.</p>
    <?php endif; ?>
</body>
</html>
