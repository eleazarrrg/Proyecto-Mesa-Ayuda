...
if (!$usuario) {
    // Por seguridad, si no encuentra el usuario, lo regresamos al login
    session_destroy();
    header('Location: index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        ...
        if ($usuarioModel->actualizarPassword($userId, $hash)) {
            ...
            // Volvemos a cargar el estudiante y la foto (por si en un futuro cambias algo ahí)
            if (!empty($usuario['email'])) {
                $estudiante = $estudianteModel->obtenerPorEmail($usuario['email']);
                if ($estudiante && !empty($estudiante['foto_perfil'])) {
                    $fotoPerfil = '../' . ltrim($estudiante['foto_perfil'], '/');
...
<body>
    ...
    <?php if ($fotoPerfil): ?>
        <div style="margin-bottom: 16px;">
            <img src="<?= htmlspecialchars($fotoPerfil, ENT_QUOTES, 'UTF-8'); ?>"
                 alt="Foto de perfil"
                 style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:3px solid rgba(0,0,0,0.1);">
        </div>
    <?php endif; ?>
